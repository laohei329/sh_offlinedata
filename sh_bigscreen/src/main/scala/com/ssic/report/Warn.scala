...
object Warn {
  ...
  def warnInsert(warnData: (RDD[SchoolBean], Broadcast[Map[String, String]], Broadcast[Map[String, Int]])) = {
    val now = format.format(new Date())
    val warn = warnData._1.filter(x => x != null && x.table.equals("t_pro_warning_global_master") && x.`type`.equals("insert") && x.data.stat.equals("1"))
  ...
  def warnInsertPeople(warnData: (RDD[SchoolBean], Broadcast[Map[String, String]], Broadcast[Map[String, Int]])) = {
    val now = format.format(new Date())
    val warn = warnData._1.filter(x => x != null && x.table.equals("t_pro_warning_global_master") && x.`type`.equals("insert") && x.data.stat.equals("1"))
    val license = warnData._1.filter(x => x != null && x.table.equals("t_pro_warning_license") && x.`type`.equals("insert") && x.data.stat.equals("1"))
  ...
  def warnUpdateDelete(warnData: (RDD[SchoolBean], Broadcast[Map[String, String]], Broadcast[Map[String, Int]])) = {
    val now = format.format(new Date())
    val warn = warnData._1.filter(x => x != null && x.table.equals("t_pro_warning_global_master") && !x.`type`.equals("insert"))
    val warnDa = warn.distinct().map({
      x =>
        val id = x.data.id
        val supplier_id = x.data.supplier_id
              ...
              if (x._2.equals(id)) {
                if (x._5.equals("0") || x._5.equals("1")) {
                  ...
                  if ("update".equals(x._1) && x._9.equals("0") && x._10.equals("1")) {
                    ...
                    jedis.hset(x._7 + "_" + "warnDetail", i, v.split("status")(0) + "status" + "_" + x._6 + "_" + "dealtime" + "_" + x._8 + "_" + "schoolid" + v.split("schoolid")(1))
