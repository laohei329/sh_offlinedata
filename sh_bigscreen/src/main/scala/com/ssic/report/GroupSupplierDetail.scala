...
object GroupSupplierDetail {
  ...
  def groupSupplierInsert(filterData: RDD[SchoolBean]) = {
    val supplierData = filterData.filter(x => x != null && x.table.equals("t_pro_supplier") && x.`type`.equals("insert") && x.data.stat.equals("1") && !("1").equals(x.data.company_type)  && ("1").equals(x.data.supplier_type))
  ...
  def groupSupplierUpDe(filterData: RDD[SchoolBean]) = {
    val supplierData = filterData.filter(x => x != null && x.table.equals("t_pro_supplier") && !x.`type`.equals("insert") && !("1").equals(x.data.company_type)  && ("1").equals(x.data.supplier_type))
    val suppilerDetailData = supplierData.distinct().map({
      x =>
        val id = x.data.id
        val supplier_type = x.data.supplier_type //1团餐公司 2供应商
