  ...
  def retentionDishDetail(filterData: RDD[SchoolBean]) = {
    ...
    val retentionDishData = retentionDish.distinct().map({
      x =>
        ...
        val now = format.format(new Date())
        if (format.parse(now).getTime <= format.parse(date).getTime) {
          (id, List(package_id, creator, date, create_time, remark,cater_type_name,menu_group_name))
        }else{
          ("null",List("null","null","null","null","null","null","null"))
        }
        ...
        itr =>
          ...
          itr.foreach({
            x =>
              ...
              jedis.hset( x._2(2)+"_"+"retention-dish", x._1+"_"+x._3(2) , "packageid"+"_" + x._2(0) +"_"+ "createtime" + "_" + x._2(3)+"_"+"creator" + "_" + x._2(1) + "_" + "quantity" + "_" + x._3(0)+ "_" + "remark" + "_" + x._2(4)+"_"+"groupname"+"_"+x._2(6)+"_"+"catertypename"+"_"+x._2(5)+"_"+"dishesname"+"_"+x._3(1))
          })
      })
  ...
  def retentionUpdateDelte(filterData: RDD[SchoolBean]) = {
    ...
    val retentionData = retention.distinct().map({
      x =>
        ...
        val now = format.format(new Date())
        if (format.parse(now).getTime <= format.parse(date).getTime){
          (id, remark, x.`type`,date, x.data.stat,cater_type_name,menu_group_name)
        }else{
              ...
              if(id.equals(x._1)){
                if ("delete".equals(x._3)) {
                  ...
                  if(StringUtils.isNoneEmpty(v) && !v.equals("null")){
                    ...
                    logger.info("留样子信息中存在这个id但是不需要计算的1111111111111--------------------")
                  }
