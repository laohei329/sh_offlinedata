...
object Receyler {
  ...
  def waste(filterData: RDD[SchoolBean]):RDD[(String, String, Int, String, String, Int, Int, String, String, String, String, String,String,Int,Int,String)] ={
    ...
    val data = recycler.map({
      ...
      val types = x.data.`type` //'1餐厨垃圾，2废弃油脂',
      ...
      val stat = x.data.stat
        ...
        if ("insert".equals(x.`type`) && "1".equals(x.data.stat)) {
          ...
          ("update", id, platform_type, source_id, district_id, types, secont_type, receiver_name, recycler_number, date, contact, stat,recycler_documents,x.old.`type`,x.old.secont_type,x.old.stat)
        } else {
  ...
  def wastedata(filterData: (RDD[SchoolBean],Broadcast[Map[String, String]],Broadcast[Map[String, String]])) ={
                  ...
                  if(strings.asScala.contains(x._2).equals(true)){
                    ...
                    if(StringUtils.isNoneEmpty(x._16) && !x._16.equals("null")){
                      ...
                      if(x._14 == 1 && x._6 == 2){
                        ...
                        if(hkeys4.asScala.contains(x._2).equals(true)){
                          jedis.hincrByFloat(x._10+"_"+"supplierwastetotal",filterData._2.value.getOrElse(x._4,"null")+"_"+"total",-1)
