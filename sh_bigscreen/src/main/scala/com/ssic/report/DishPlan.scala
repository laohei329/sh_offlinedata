...
import com.ssic.beans.{SaasPackage, SaasPackageDishes, SchoolBean}
...
object DishPlan {
  ...
  def Dish(filterData: RDD[SchoolBean]) = {
    ...
    val packages = filterData.filter(x => x != null && x.table.equals("t_saas_package") && x.`type`.equals("insert") )
      ...
      .filter(x =>  !x.stat.equals("0") && x.is_publish != 0)
    ...
    val dishFlitData = dishData.distinct().map(x => (x.package_id, x.dishes_name, x.dishes_number)).filter(x => StringUtils.isNoneEmpty(x._1)).filter(x => StringUtils.isNoneEmpty(x._2)).map(x => (x._1, x._2 + ";" + x._3))
