      ...
      val stat = x.data.stat
        ...
        if(format.parse(now).getTime <= format.parse(date).getTime){
          if (x.`type`.equals("insert") && stat.equals("1")) {
            (id, List(date, ledger_type, receiver_id, source_id, ware_batch_no, haul_status, "null", "insert", stat,"null"))
          } else if (x.`type`.equals("update")) {
            (id, List(date, ledger_type, receiver_id, source_id, ware_batch_no, haul_status, x.old.haul_status, "update", stat,x.old.stat))
          } else {
            (id, List(date, ledger_type, receiver_id, source_id, ware_batch_no, haul_status, "null", "delete", stat,"null"))
          }
        }else{
