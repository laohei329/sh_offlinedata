...
public class MatConfirmDetsAppMod {
	...
	String[] optMode_Array = {"外包-现场加工", "外包-现场加工", "自营", "外包-现场加工", "外包-现场加工", "外包-现场加工", "外包-快餐配送", "外包-现场加工", "外包-现场加工", "外包-现场加工", "外包-现场加工", "外包-现场加工", "外包-现场加工"};
	...
	private MatConfirmDetsDTO matConfirmDets(String distIdorSCName, String[] dates, List<TEduDistrictDo> tddList, 
			...
			if (useMaterialPlanDetailMap != null) {
				for (String curKey : useMaterialPlanDetailMap.keySet()) {
					...
					if(curKeys.length >= 10) {
	    				...
	    				mcd.setCompDep("其他");
	    				if(curSubLevel == 0) {      //其他
	    					if(tesDo.getDepartmentSlaveId() != null && CommonUtil.isInteger(tesDo.getDepartmentSlaveId())) {
		...
		mcdDto.setTime(BCDTimeUtil.convertNormalFrom(null));
		//分页
		PageBean<MatConfirmDets> pageBean = new PageBean<MatConfirmDets>(matConfirmDets, curPageNum, pageSize);
		PageInfo pageInfo = new PageInfo();
