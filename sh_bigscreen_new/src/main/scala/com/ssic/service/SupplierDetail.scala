  ...
  def supplierDetail(filterData: RDD[SchoolBean]) ={
    ...
    val supplierDetail = supplietData.distinct().map({
      ...
      val supplier_classify = x.data.supplier_classify // '供应商分类  0 不区分 1生产类 2 经销类',
        val region_id = Rule.emptyToNull(x.data.region_id)
        ...
        val business_license = Rule.emptyToNull(x.data.business_license_number) //营业执照号码
      val reg_capital =Rule.emptyToNull( x.data.registered_capital) //注册资金
      val food_business_code = "null" //食品经营许可证号
      val food_circulation_code =  "null" //食品流通证号
      val food_produce_code =  "null" //食品生产证号
        //新的中台数据没有stat这个字段
        var stat="null"
        if("1".equals(x.data.is_available) && "0".equals(x.data.is_deleted)){
          stat="1"
        }else{
          stat="0"
        }
