...
object SchoolDetail {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val groupon_customer = session.read.jdbc(url, edu_groupon_customer, conn)
    ...
    val schoolRdd: RDD[(String, String)] = schoolRDD.leftOuterJoin(groupon_customerRDD).map {
      rdd =>
        val id: String = rdd._1
        val school: String = rdd._2._1
        val cus: Option[String] = rdd._2._2
//        var license_main_type:String=null;
//        var license_main_child:String=null;
        if (None.equals(cus)) {
          (id, school+"catering_new_type;"+null+";")
        } else {
/*          val canteenmode_before: String = school.split("canteenmode;")(0)
          val license_main_child_after: String = school.split(";studentsamount")(1)
          if(cus=="0"){
            license_main_type="0"
          }else if(cus=="1"){
            license_main_type="1"
          ...
          (id, school+"catering_new_type;"+cus.get+";")
        }
    }
