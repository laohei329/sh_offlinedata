...
object DayToExcelDepartment {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 until departmentid.size) {
      ...
      val edu_platoon_detail = hiveContext.sql(s"select * from app_saas_v1.app_t_edu_platoon_detail where year ='${year}' and month ='${month}' and use_date ='${date}' and department_id ='${id}'  ").rdd
        .map({
          row =>
      ...
      val licenseWarnData = hiveContext.sql(s"select * from app_saas_v1.app_t_edu_warn_detail where warn_type = 1  and target = 3 and warn_date = '${date}'  and department_id ='${id}' ").rdd.map({
        row =>
          val department_id = row.getAs[String]("department_id")
          val department_name = departmentidToName.value.getOrElse(department_id, "null")
          val address = Rule.nullToEmpty(row.getAs[String]("address"))
      ...
      //排菜
      val qu_tuples = edu_platoon_detail.map(x => (x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._9, x._10)).sortBy(x => (x._1, x._6)).collect()
      ...
      val filename = year + "年食安管理平台使用、验收及证照逾期处理情况_" + departName + "_" + datetime + "__" + datetime + "__" + date3 + "000000" + ".xls"
      ...
      new ShanghaiWeekReport().areatotal(sheet, edu_platoon_detail.map(x => (x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._9, x._10)), workbook, edu_platoon_detail.map(x => (x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._11, x._12)), date, date, licenseWarnData, date2, year)
      ...
      //配送
      val qu_ledger_tuples = edu_platoon_detail.map(x => (x._1, x._2, x._3, x._4, x._5, x._6, x._7, x._8, x._11, x._12)).sortBy(x => (x._1, x._6)).collect()
