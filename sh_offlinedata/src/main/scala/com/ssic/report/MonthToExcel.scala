...
import com.ssic.report.DayToExcel.format
import com.ssic.report.MonthToExcelDepartment._
...
object MonthToExcel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val departmentidToName: Broadcast[Map[String, String]] = sc.broadcast(Tools.departmentidToName(sparkSession))
      ...
      val edu_platoon_detail = hiveContext.sql(s"select * from app_saas_v1.app_t_edu_platoon_total_m where year='${year}' and month ='${month}' ").rdd.map({
        row =>
      ...
      val ledgerData = hiveContext.sql(s"select * from app_saas_v1.app_t_edu_ledger_master_total_m where year ='${year}' and month ='${month}'  ").rdd.map({
        row =>
          val department_id = row.getAs[String]("department_id")
          val department_name = departmentidToName.value.getOrElse(department_id, "null")
          val school_name = row.getAs[String]("school_name")
      ...
      val licenseWarnData = hiveContext.sql(s"select * from app_saas_v1.app_t_edu_warn_detail where warn_type = 1 and target = 3 and year='${year}' and month ='${month}'  ").rdd.map({
        row =>
          val department_id = row.getAs[String]("department_id")
          val department_name = departmentidToName.value.getOrElse(department_id, "null")
          val address = Rule.nullToEmpty(row.getAs[String]("address"))
      ...
      val filename = year + "年食安管理平台使用、验收及证照逾期处理情况_上海市_" + year + month2 + "01" + "__" + datetime1 + "__" + date3 + "000000" + ".xls"
