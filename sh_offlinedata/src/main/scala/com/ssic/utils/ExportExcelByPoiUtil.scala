...
object ExportExcelByPoiUtil {
  ...
  def createExcelMerge(title: Array[String], titleHead: String, widthAttr: Array[Int],
                       maps: Map[String /*sheet名*/ , List[Map[String /*对应title的值*/ , String]]],
                       mergeIndex: Array[Int], fileOutputStream: FileOutputStream) = {
    if (title.length == 0) null
    ...
    for (elem <- maps) {
      ...
      // 合并单元格CellRangeAddress构造参数依次表示起始行，截至行，起始列， 截至列
      /*第一行是标题  设置成合并*/
      val c1 = new CellRangeAddress(0, 0, 0, title.length - 1);
      sheet.addMergedRegion(c1)
      /*第二行也是合并*/
      val c2 = new CellRangeAddress(1, 1, 0, title.length - 1)
      sheet.addMergedRegion(c2)
      ...
      // 设置单元格内容 标题
      cell1.setCellValue("单元格标题———" + titleHead + "一览表")
