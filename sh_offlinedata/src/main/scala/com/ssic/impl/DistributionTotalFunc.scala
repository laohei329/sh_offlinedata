...
trait DistributionTotalFunc {
  ...
  def canteenmodedistributionschoolstatus(data:(RDD[(String, String)],String,RDD[(String, String)],Broadcast[Map[String, List[String]]]))

  //按权限的上海市经营模式来配送计划各状态按照学校数据统计（canteenmode,对学校进行去重处理）
  def departmentcanteenmodedistributionschoolstatus(data:(RDD[(String, String)],String,Broadcast[Map[String, List[String]]]))
